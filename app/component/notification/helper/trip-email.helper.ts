...
export interface TripCanceledInterfacAdminEmailInterface {
  firstName: RiderInterface['firstName'];
  dropOffLocation: string;
  pickUpLocation: string;
}
const TripEmailHelper = {
  ...
  createTripCancelledEmail(data: TripCanceledInterfacAdminEmailInterface) {
    const { firstName, dropOffLocation, pickUpLocation } = data;
    return {
      to: EmailHelper.gettripCancelledEmail(),
      subject: EMAIL_SUBJECT.NEW_TRIP_CANCELED,
      html: EmailTemplatesHelper.generateTemplate(
        {
          firstName,
          dropOffLocation,
        ...
        'trip-cancelled-admin.html',
      ),
      from: EmailHelper.getFromEmail(),
    };
  },
  createTripCancelledEmailToAdmin(
    data: TripCanceledInterfacUserEmailInterface,
  ) {
    const { firstName, dropOffLocation, pickUpLocation, email } = data;
    return {
      to: EmailHelper.gettripCancelledEmail(),
      subject: EMAIL_SUBJECT.NEW_TRIP_CANCELED,
      html: EmailTemplatesHelper.generateTemplate(
