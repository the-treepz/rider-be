...
class VerificationController {
  public requestToVerifyEmail = async (
    ...
    const user = await RiderService.findOne({ email: request.body.email }, true);
  ...
  public verifyEmail = async (request: Request, response: Response) => {
    const findUser = await RiderService.findOne({ otp: request.body.otp }, true);
    if (findUser.status=== USER_STATUS_ENUM.ACTIVE) throw new ClientError('user previouly verified');
    ...
    return RiderService.update(findUser._id, {status: USER_STATUS_ENUM.ACTIVE})
  };
}
